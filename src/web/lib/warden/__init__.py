...
# -*- coding: utf-8 -*-
#
# This file is from https://warden.cesnet.cz/cs/downloads and is not a direct part of VA2AM.
# It is not a dependency managed by pip, because the Warden Client is not on PyPy.
...
class Client(object):
        ...
        if filelog is not None:
            try:
                fl = logging.FileHandler(
                    filename=path.join(path.dirname(__file__), filelog.get("file", "%s.log" % self.name)), encoding="utf-8"
                )
        ...
        if syslog is not None:
            try:
                sl = logging.handlers.SysLogHandler(
                    address=fix_logging_filename(syslog.get("socket", "/dev/log")), facility=facility(syslog.get("facility", "local7"))
                )
    ...
    def connect(self):
        ...
        try:
            if self.url.scheme == "https":
                if self.sslcontext:
                    conn = httplib.HTTPSConnection(self.url.netloc, timeout=self.timeout, context=self.sslcontext)
                else:
                ...
                conn = httplib.HTTPConnection(self.url.netloc, timeout=self.timeout)
            else:
                return Error(message='Don\'t know how to connect to "%s"' % self.url.scheme, url=self.url.geturl())
        except Exception:
            return Error(
    ...
    def sendRequest(self, func="", payload=None, **kwargs):
        ...
        if payload is None:
            ...
            try:
                ...
                return Error(message="Serialization to JSON failed", exc=exc_info(), method=func, payload=payload)

            self.headers.update({"Content-Type": "application/json", "Content-Length": str(len(data))})
        ...
        try:
            ...
            conn.close()
            return Error(message="Sending of request to server failed", exc=exc_info(), method=func, log=loc, headers=self.headers, data=data)

        try:
            res = conn.getresponse()
        except:
            conn.close()
            return Error(method=func, message="HTTP reply failed", exc=exc_info(), loc=loc, headers=self.headers, data=data)
        ...
        try:
            ...
            conn.close()
            return Error(
                method=func, message="Fetching HTTP data from server failed", exc=exc_info(), loc=loc, headers=self.headers, data=data
            )
        ...
        if res.status == httplib.OK:
            try:
                ...
                data = Error(method=func, message="JSON message parsing failed", exc=exc_info(), response=response_data)
        else:
            try:
                data = json.loads(response_data.decode("utf-8"))
                data["errors"]  # trigger exception if not dict or no error key
            except:
                data = Error(method=func, message="Generic server HTTP error", error=res.status, exc=exc_info(), response=response_data)
            else:
                data = Error(method=data.get("method", None), req_id=data.get("req_id", None), errors=data.get("errors", []))
    ...
    def _saveID(self, id, idstore=None):
        ...
        try:
            ...
            Error(message='Writing id file "%s" failed' % idf, exc=exc_info(), idstore=idf).log(self.logger, logging.INFO)
        return id
    ...
    def _loadID(self, idstore=None):
        ...
        try:
            ...
            Error(message='Reading id file "%s" failed, relying on server' % idf, exc=exc_info(), idstore=idf).log(self.logger, logging.INFO)
            id = None
        return id
    ...
    def getEvents(self, id=None, idstore=None, count=None, cat=None, nocat=None, tag=None, notag=None, group=None, nogroup=None):
        ...
        res = self.sendRequest(
            "getEvents",
            id=id,
            count=self.get_events_limit if count is None else count,
            cat=cat,
            nocat=nocat,
            tag=tag,
            notag=notag,
        ...
        if res:
            try:
                ...
                events = Error(method="getEvents", message="Server returned bogus reply", exc=exc_info(), response=res)
            self._saveID(newid)
        else:
            events = res
